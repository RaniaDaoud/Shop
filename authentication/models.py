...
class Profile(models.Model):
    ...
    def notify_liked(self, feed):
        if self.user != feed.user:
            Notification(type=Notification.LIKED,
                         from_user=self.user, to_user=feed.user,
                         feed=feed).save()

    def unotify_liked(self, feed):
        if self.user != feed.user:
            Notification.objects.filter(type=Notification.LIKED,
                                        from_user=self.user, to_user=feed.user,
                                        feed=feed).delete()

    def notify_commented(self, feed):
        if self.user != feed.user:
            Notification(type=Notification.COMMENTED,
                         from_user=self.user, to_user=feed.user,
                         feed=feed).save()
    ...
    def notify_also_commented(self, feed):
        ...
        for user in users:
            Notification(type=Notification.ALSO_COMMENTED,
                         from_user=self.user,
                         to_user=User(id=user), feed=feed).save()

    def notify_favorited(self, question):
        if self.user != question.user:
            Notification(type=Notification.FAVORITED,
    ...
    def notify_answered(self, question):
        if self.user != question.user:
            Notification(type=Notification.ANSWERED,
                         from_user=self.user,
                         to_user=question.user,
                         question=question).save()
    ...
    def notify_accepted(self, answer):
        if self.user != answer.user:
            Notification(type=Notification.ACCEPTED_ANSWER,
                         from_user=self.user,
                         to_user=answer.user,
                         answer=answer).save()
    ...
    def unotify_accepted(self, answer):
        if self.user != answer.user:
            Notification.objects.filter(
                type=Notification.ACCEPTED_ANSWER,
                from_user=self.user,
                to_user=answer.user,
